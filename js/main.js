...
const cases = [
  ...
  },
  {
    ...
    cardText: 'Создать убедительный лендинг для компании по комплексному ремонту квартир, который показывает прозрачную смету, этапы и контроль качества и помогает получить заявку на предварительный расчёт.',
    result: 'Получился подробный лендинг для услуги с высоким чеком: премиальная визуальная подача поддерживает доверие, а портфолио, расчёт бюджета и прозрачный процесс последовательно ведут пользователя к заявке.',
    ...
    features: ['7-шаговый квиз', 'Сравнение «до/после»', 'Портфолио интерьеров', 'Прозрачная смета'],
    blocks: ['Первый экран', 'Направления ремонта', 'Портфолио', 'До и после', 'Квиз-калькулятор', 'Стоимость', 'Процесс', 'Гарантии', 'FAQ', 'Форма заявки'],
    functionality: ['Квиз-калькулятор', 'Before/after-слайдер', 'Формы заявки', 'Telegram-интеграция', 'Адаптив'],
  },
  {
    id: 'mos-reg-guide',
